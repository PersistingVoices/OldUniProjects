...
#pragma omp parallel for default(none) private(i,j) shared(a,b) schedule( dynamic, 4)
	for (i=0; i<N; i++){ 
		for (j=N-1; j>i; j--){
			a[i][j] += cos(b[i][j]);
		} 
...
#pragma omp parallel for default(none) private(i,j,k) shared(a,b,c,jmax,rN2) schedule( dynamic, 4)
	for (i=0; i<N; i++){ 
		for (j=0; j < jmax[i]; j++){
			for (k=0; k<j; k++){ 
