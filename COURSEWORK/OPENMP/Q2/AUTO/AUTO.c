...
int main(int argc, char *argv[]) { 
	...
	pFile1 = fopen ("Data/AUTO-file1.dat" , "w");
	pFile2 = fopen ("Data/AUTO-file2.dat" , "w");
	...
	printf("\n\t\t\tWritten to 'AUTO-file1.dat & AUTO-file2.dat'\n\n"); 
	fclose(pFile1);
	fclose(pFile2);
	...
	fputs("splot 'Data/AUTO-file1.dat' u 1:2:3 w lines \n ", pipe_gp1);
	fputs("set terminal png  size 1200,800\n ", pipe_gp1); 
	fputs("set output 'Plots/AUTO-3dplot1.png'\n ", pipe_gp1);
	fputs("replot\n ", pipe_gp1);
	...
	fputs("plot 'Data/AUTO-file1.dat' u 2:3 w lines \n ", pipe_gp1);
	fputs("set terminal png  size 1200,800\n ", pipe_gp1); 
	fputs("set output 'Plots/AUTO-2dplot1.png'\n ", pipe_gp1);
	fputs("replot\n ", pipe_gp1);
	...
	fputs("set ylabel 'Iteration Number'\n ", pipe_gp1);
	fputs("set zlabel 'Time taken for 100 iterations'\n ", pipe_gp1);
	fputs("splot 'Data/AUTO-file2.dat' u 1:2:3 w lines \n ", pipe_gp1);
	fputs("set terminal png  size 1200,800\n ", pipe_gp1); 
	fputs("set output 'Plots/AUTO-3dplot2.png'\n ", pipe_gp1);
	fputs("replot\n ", pipe_gp1);
	...
	fputs("set ylabel 'Iteration Number'\n ", pipe_gp1);
	fputs("plot 'Data/AUTO-file2.dat' u 2:3 w lines \n ", pipe_gp1);
	fputs("set terminal png  size 1200,800\n ", pipe_gp1); 
	fputs("set output 'Plots/AUTO-2dplot2.png'\n ", pipe_gp1);
	fputs("replot\n ", pipe_gp1);

	pclose(pipe_gp1);

	printf("\n\t\t\tPlots at 'AUTO--plot1.png & AUTO--plot2.png' for loop1 & loop2  (100I)\n\n"); 
} 
...
#pragma omp parallel for default(none) private(i,j) shared(a,b ) schedule(auto)
	for (i=0; i<N; i++){ 
		for (j=N-1; j>i; j--){
			a[i][j] += cos(b[i][j]);
		} 
...
#pragma omp parallel for default(none) private(i,j,k) shared(a,b,c,jmax,rN2) schedule(auto)
	for (i=0; i<N; i++){ 
		for (j=0; j < jmax[i]; j++){
			for (k=0; k<j; k++){ 
